...
fw = open('log.%s_newton_epsilon_%f'%(data_n,epsilon),'w')
if fw is False:
...
print()
print("=============================")
print("Newton optimization starts...")
print("=============================")
print()
...
while count < max_count and norm_df > epsilon*np.maximum(1,norm_sol):
    ...
    H = Functions.hessian(sol,N,M,xtrain,ytrain,lambda_1,lambda_2)
    delta_sol = np.linalg.solve(H,df)
    sol += delta_sol
    df = Functions.d_function(sol,N,M,xtrain,ytrain,lambda_1,lambda_2)
    f_value = Functions.target_function(sol,N,M,xtrain,ytrain,lambda_1,lambda_2)
    norm_df = np.sqrt(np.dot(df,df))
...
f_his.append(f_value)
np.savetxt('%s_newton_solution_epsilon_%f.dat'%(data_n,epsilon),sol.reshape(len(sol),1))
np.savetxt('%s_newton_f_history_epsilon_%f.dat'%(data_n,epsilon),np.array(f_his).reshape(len(f_his),1))
...
if count != max_count:
    ...
    print()
    print("=================================")
    print("Newton optimization is completed!")
    ...
    print("======================================")
    print("              WARNING                 ")
    print("Newton optimization fails to converge!")
    print("Data set: %s"%data_n)
